...
ANFParser::ANFParser(char *filename)
{
	...
	else
	{
		...
		while (perspectiveElement)
		{
			...
			cameras[p_camera->getId()]=*p_camera;
			if(!initial_camera) {
		...
		while (orthoElement)
		{
			...
			cameras[o_camera->getId()]=*o_camera;
			if(!initial_camera)
	...
	else
	{
		...
		else
		{
			...
			while (textureElement){
				...
				myTexture *newTexture = new myTexture(id, file);
		...
		else
		{
			...
			while (appearanceElement)
			{
				...
				newAppearance = new CGFappearance(amb, dif, spe, shininess);
				if(!textureref){

				}
				else
					newAppearance->setTexture(textureref);
...
std::map<char*,CGFcamera> ANFParser::getCameras(){
	return cameras;
}
std::map<char*,CGFtexture> ANFParser::getTextures(){
	return textures;
}
std::map<char*,CGFappearance> ANFParser::getAppearances(){
	return appearances;
}

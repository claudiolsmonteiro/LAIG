...
void DemoScene::processNode(string nodeid, string inh){
    ...
    if(parser->getGraph().getNodes()[i].isDisplayList())
        glCallList(parser->getGraph().getNodes()[i].dlist);
    else {
...
void DemoScene::CreateDisplayList(string n,string a) {
    ...
    if(parser->getGraph().getNodes()[i].isDisplayList()) {
        parser->getGraph().getNodes()[i].dlist = glGenLists(1);
        glNewList(parser->getGraph().getNodes()[i].dlist, GL_COMPILE);
    }

    Draw(parser->getGraph().getNodes()[i],a);
    if(parser->getGraph().getNodes()[i].isDisplayList())
        glEndList();
       
    for(unsigned int y = 0; y < parser->getGraph().getNodes()[i].getDescendants().size(); y++){
        CreateDisplayList(parser->getGraph().getNodes()[i].getDescendants()[y], a);
    }
